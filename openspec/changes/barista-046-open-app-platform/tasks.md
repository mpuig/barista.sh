...
- [ ] 3.1 Extend the Runtime trait with a fork result that reports actual mode and source-freeze details without importing substrate-specific types.
- [ ] 3.2 Implement journaled `ForkInstance` validation, immutable source-spec cloning, target creation, lineage events, idempotent replay, and cleanup.
- [ ] 3.3 Add a full-copy reference path and fail-closed `require_cow` behavior.
...
- [ ] 3.5 Add integration tests for two divergent children, unchanged source, duplicate targets, replayed keys, capability refusal, and kill -9 recovery.

## 4. Capsule export, import, and remote tier

- [ ] 4.1 Implement verify-then-publish capsule export from retained snapshots using the local immutable-object backend.
- [ ] 4.2 Implement staged capsule import, version/integrity checks, compatibility preflight, and registration without boot.
...
- [ ] 4.5 Add tests for tamper, truncation, missing objects, CPU/template/bundle mismatch, source-node loss, upload crash, retry, and shared-object deletion.
...
- [ ] 5.5 Add tests proving sibling epoch separation, old-epoch refusal, no persistent carrier, readiness ordering, and honest warnings that arbitrary workload memory remains sensitive.
