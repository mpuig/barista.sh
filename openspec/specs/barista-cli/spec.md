...
The `barista` CLI SHALL expose every Contract A verb (`create`, `start`, `stop`,
...
- **WHEN** `barista pause <id>` runs against a running instance on a runtime with
  `memory_snapshot`
- **THEN** the command streams progress events and exits 0 once the instance is
  `PAUSED`

#### Scenario: failure surfaces the reason
- **WHEN** `barista checkpoint <id>` targets a fake-runtime instance
- **THEN** the command exits non-zero and prints reason `CAPABILITY_MISSING`

### Requirement: Guest passthrough from the CLI
`barista exec` SHALL provide interactive (PTY) and scripted (pipe) execution with
exit-code propagation; `barista cp` SHALL copy files in and out of an instance.

#### Scenario: exit code propagation
- **WHEN** `barista exec <id> -- sh -c 'exit 42'` runs
...
a digest-pinned session image, performs work via `barista exec`, pauses, resumes,
...
`barista doctor` SHALL ask the node it is pointed at — over Contract A, not the
...
- **WHEN** `barista doctor` runs against a node whose substrate is unreachable or
