...
/// Whether an idle-pause is due (barista-037). Pure, so the policy is testable
/// without a substrate: a running instance whose last activity is older than a
/// non-zero window is due; window `0` opts out; a non-running instance is never
/// due. `saturating_sub` so a clock briefly ahead of `now` cannot wrap.
pub(crate) fn idle_pause_due(
    running: bool,
...
/// Enforce a desired record's `idle_pause_s` for one owned session (barista-037).
///
/// A `RUNNING` session idle past its window is paused through the ordinary ops
/// path; wake-on-request resumes it on the next call, so the pause is transparent.
/// The idle clock is node-side (`Agent::last_activity_ms`): seeded the first time
/// this is seen running, reset by user activity via `note_activity`, and forgotten
/// when the session is not running or once a pause is fired — so a resumed run
/// starts counting fresh.
fn enforce_idle_pause(agent: &Arc<Agent>, window_s: u32, instance_id: &str) {
    if window_s == 0 || instance_id.is_empty() {
        return;
    }
    let id = InstanceId::from(instance_id.to_string());

    let running = matches!(
        agent.db.get_instance(&id),
        Ok(Some(row)) if row.state == pb::InstanceState::Running
    );
    if !running {
        // Paused already, gone, or never started: forget the clock so a later run
        // begins its window afresh.
    ...
    let now = now_ms();
    let last = match agent.last_activity_ms.lock() {
        Ok(mut clocks) => *clocks.entry(id.clone()).or_insert(now),
        Err(_) => return,
    };
    if !idle_pause_due(true, last, now, window_s) {
        return;
    }

    // Idle past the window. Key the pause on the window's opening so a re-tick
    // before the pause lands binds to the same operation rather than queueing a
    // second; once the instance leaves RUNNING the guard above stops any more.
    let key = IdempotencyKey::from(format!("idle_pause:{id}:{last}"));
    let announce = |op_id: &OpId| {
        agent.events.idle_fired(
            &id,
            op_id,
            &format!(
                "session idle beyond idle_pause_s={window_s}s; pausing (wake-on-request resumes)"
            ),
        );
    };
    match crate::ops::submit_claiming(
        agent,
        crate::ops::OpKind::Pause,
        &id,
        &key,
        crate::ops::OpPayload::Pause {
            require_memory: false,
        },
        None,
        &announce,
    ) {
        Ok(crate::ops::Claimed::Submitted(submitted)) => {
            info!(instance = %id, window_s, op = %submitted.op.op_id, "idle_pause fired");
            // Forget the clock; the resume that wakes it re-seeds on next sight.
            if let Ok(mut clocks) = agent.last_activity_ms.lock() {
                clocks.remove(&id);
            }
        }
        // A claimless submission cannot be superseded; kept exhaustive.
        Ok(crate::ops::Claimed::Superseded) => {}
        // Something else is mutating the instance; the next pass retries once it
        // settles, and the clock is left in place so the window is not lost.
        Err(e) if e.reason == pb::ErrorReason::ConcurrentOperation => {}
        Err(e) => warn!(instance = %id, %e, "idle_pause could not be submitted"),
    }
}
...
pub async fn pass(agent: &Arc<Agent>, fleet: &Fleet) -> PassReport {
    ...
    for want in desired {
        ...
        // barista-037: a running session idle past its record's idle_pause_s is
        // paused here — transparently, since wake-on-request resumes it on the
        // next call. Before materialisation, because a session we already hold and
        // run is exactly the one the timeout applies to; a just-acquired or paused
        // instance is not RUNNING, so the call returns early.
...
mod tests {
    ...
    /// The idle policy is a pure predicate, exhausted here without a substrate.
    #[test]
    fn idle_pause_due_is_a_pure_policy() {
        // Opt-out: window 0 is never due, however idle.
        assert!(!idle_pause_due(true, 0, 10_000_000, 0));
        // A non-running instance is never due.
        assert!(!idle_pause_due(false, 0, 10_000_000, 5));
        // Within the window: 4s idle against a 5s window.
        assert!(!idle_pause_due(true, 6_000, 10_000, 5));
        // Exactly at the window is not yet past it.
        assert!(!idle_pause_due(true, 5_000, 10_000, 5));
        // Past the window: 6s idle against a 5s window.
        assert!(idle_pause_due(true, 4_000, 10_000, 5));
    }

    async fn agent_with_running(dir: &tempfile::TempDir, id: &str) -> Arc<Agent> {
        let agent = Agent::bootstrap(
            crate::Config::from_env(dir.path().to_path_buf()),
            Arc::new(StubRuntime::default()),
        )
    ...
    fn state_of(agent: &Arc<Agent>, id: &str) -> pb::InstanceState {
        agent
            .db
    ...
    fn has_clock(agent: &Arc<Agent>, id: &str) -> bool {
        agent
            .last_activity_ms
            .lock()
            .unwrap()
            .contains_key(&InstanceId::from(id.to_string()))
    }

    /// A running session whose idle clock is older than its window is paused —
    /// observable as PAUSING, which `submit_claiming` writes at submission — and
    /// its clock is forgotten so the resume that wakes it re-seeds fresh.
    #[tokio::test]
    async fn idle_past_the_window_pauses_the_session() {
        let dir = tempfile::tempdir().unwrap();
        let agent = agent_with_running(&dir, "idle-1").await;
        agent
            .last_activity_ms
            .lock()
            .unwrap()
            .insert(InstanceId::from("idle-1".to_string()), now_ms() - 10_000);

        enforce_idle_pause(&agent, 5, "idle-1");

        assert_eq!(
            state_of(&agent, "idle-1"),
            pb::InstanceState::Pausing,
            "a session idle past its window must be paused"
        );
        assert!(
            !has_clock(&agent, "idle-1"),
            "a fired pause forgets the clock so the resume re-seeds it"
        );
    }

    /// A first sight seeds the clock to `now`, so a freshly-seen session is not
    /// paused and gets a baseline for next time.
    #[tokio::test]
    async fn a_fresh_clock_does_not_pause() {
        let dir = tempfile::tempdir().unwrap();
        let agent = agent_with_running(&dir, "fresh-1").await;

        enforce_idle_pause(&agent, 5, "fresh-1");

        assert_eq!(state_of(&agent, "fresh-1"), pb::InstanceState::Running);
        assert!(
            has_clock(&agent, "fresh-1"),
            "first sight must seed the idle clock"
        );
    }

    /// `idle_pause_s = 0` opts out however long the session has been idle.
    #[tokio::test]
    async fn window_zero_never_pauses() {
        let dir = tempfile::tempdir().unwrap();
        let agent = agent_with_running(&dir, "optout-1").await;
        agent.last_activity_ms.lock().unwrap().insert(
            InstanceId::from("optout-1".to_string()),
            now_ms() - 10_000_000,
        );

        enforce_idle_pause(&agent, 0, "optout-1");

        assert_eq!(state_of(&agent, "optout-1"), pb::InstanceState::Running);
    }
}
